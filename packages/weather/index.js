...
const apiKey = 'e7119769148e312eba11f16cdd5bd747';

let cityQuery = 'Moscow';
...
function weather(query) {
    ...
    function filtrCities(cities){ // check if there is more than one city with the same name
        cities.forEach(city => {
            if(city.name === cityQuery){
                if(!countryList.has(city.country)) {
                    countryList.add(city.country);
    ...
    return new Promise(res => {
        fs.readFile(__dirname + "/city-list.json", 'UTF-8', (err, data) => {
            if (err) throw err;
