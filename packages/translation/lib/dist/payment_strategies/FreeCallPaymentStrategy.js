...
var _state_service_grpc_pb = _interopRequireDefault(require("../proto/state_service_grpc_pb"));
...
class FreeCallPaymentStrategy {
  constructor(serviceClient) {
    ...
    this._encodingUtils = new _sdkCore.EncodingUtils();
  }
  /**
  ...
  async isFreeCallAvailable() {
    try {
      const freeCallsAvailableReply = await this._getFreeCallsAvailable(); // Bypassing free calls if the token is empty
  ...
  async _getFreeCallStateRequest() {
    ...
    const {
      ...
      signature,
      currentBlockNumber
    } = await this._getFreeCallStateRequestProperties(); //  if the token for freecall is empty, then user is taken to paid call directly
  ...
  _generateFreeCallStateServiceClient() {
    ...
    return new _state_service_grpc_pb.default.FreeCallStateServiceClient(serviceEndpoint.host, grpcCredentials);
  }
  /**
...
var _default = FreeCallPaymentStrategy;
