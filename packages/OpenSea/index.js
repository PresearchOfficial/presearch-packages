...
async function trigger(query) {
  ...
  // console.log(assets);
  const q = query.toLowerCase().split(' ');
  if (assets.some((asset) => asset.name && q.some((el) => asset.name.toLowerCase().indexOf(el) > -1))) {
    return true;
  }
  let filteredAssets = assets.filter(
    (asset) =>
      asset.asset_contract.name && q.every((el) => asset.asset_contract.name.toLowerCase().indexOf(el) > -1)
  );
  if (filteredAssets.length) {
    return true;
  }
  filteredAssets = assets.filter((asset) =>
    asset.traits.some((trait) => q.some((el) => trait.trait_type.toLowerCase().indexOf(el) > -1))
  );
  if (filteredAssets.length) {
    return true;
  }
